...
<div class="model" id="deletemodal">
    <div class="model-content">
...
<script>
    ...
    // function deletemodal(url, redirecturl) {
    //     // $('#deletemodal .modal-title').html(title);
    //     // $('#settings').modal('show');
    //     var thisModel = $("#deletemodal").show();
...
// Function to serialize and submit forms
                ...
                $.ajax({
                    ...
                    error: function (jqXHR, textStatus, errorThrown) {
                        ...
                        if (jqXHR.status === 0) {
                            ...
                            errorMessage = `Client Error: ${jqXHR.responseJSON?.message || jqXHR.statusText}`;
    ...
    function openModal(url, title, viewonly=false) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#settinsgs .modal-body').html(data);
                initTinyMCE();
                if (viewonly) {
                    $('#settinsgs :input').prop('readonly', true);
                    $('#settinsgs select').prop('disabled', true);
                    $('#settinsgs textarea').prop('readonly', true);
                    $('#settinsgs :checkbox').prop('disabled', true);
                    $('#settinsgs [type="submit"]').remove();
            ...
            error: function(xhr, status, error) {
                ...
                var errorMessage = xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'An error occurred while loading the content.';
                var errorDetails = xhr.responseJSON && xhr.responseJSON.details ? xhr.responseJSON.details : error;

                // Display SweetAlert error message with HTML content
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
