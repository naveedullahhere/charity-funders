                ...
                @if ($value->parent_id === null)
                    <div class="permission-container errorappend list-group">
                        <div class="custom-accordion row w-100 mx-auto py-0">
                            <label class="col-11 d-flex align-items-center  py-2 text-uppercase"
                                style="font-size:14px;gap:4px">{{ Form::checkbox('permission[]', $value->name, false, ['class' => 'parent']) }}
                        ...
                        <div class="sub-permissions" style="display: none">
                            @foreach ($permission as $subPermission)
                                @if ($subPermission->parent_id == $value->id)
                                    <label
                                        class="pl-5 py-2">{{ Form::checkbox('permission[]', $subPermission->name, false, ['class' => 'child']) }}
